...
/**
 * Mybatis - 通用分页拦截器
 * 			备注:sqlserver 2008一定要在原sql中添加 row_number() over (order by column asc/desc) as rownum这句话，
 * 			否则分页出错
 *
...
public class PageHelper implements Interceptor {
     ...
     * @param pageSize
     */
    public static void startPage(Integer pageNum, Integer pageSize, Boolean count) {
    ...
    private String getPageSql(String sql, Page page) {
        ...
        if ("mysql".equals(dialect)) {
            ...
            pageSql.append(sql);
            pageSql.append(" ) temp where rownum > ").append(page.getStartRow());
            pageSql.append(" and rownum <= ").append(page.getEndRow());
        }
